...
class IntrospectClassifier(BaseStructuredOutputExtractor):
    ...
    SYSTEM_PROMPT = """\
        ...
        **决策指南:**
        - 如果 **正确性** 或 **流畅性** 评分 **都低于 3 分**，说明回复有重大缺陷，最终决策应输出“{IntrospectLayer}”（重试），并**必须**在 reason 字段中简要说明未通过的原因（例如：未回答用户核心问题、表述生硬、事实错误、工具报错等），以便下一轮生成时改进。
        - 如果 **正确性** 和 **流畅性** 评分 **都达到或超过 3 分**，说明回复质量足够高，最终决策应输出“{FinalResponseLayer}”（接受），reason 可留空。
