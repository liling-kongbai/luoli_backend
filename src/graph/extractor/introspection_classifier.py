...
class IntrospectionClassifier(BaseStructuredOutputExtractor):
    ...
    SYSTEM_PROMPT = """\
        ...
        **评估指南：**
        请从以下维度进行 1 - 5 分的评分（1 = 差，3 = 合格，5 = 优秀）。
            1. **正确性**：回复是否准确、真实地解决了用户的问题？
            2. **流畅性**：回复是否流畅、自然、符合人设、符合上下文？
            3. 如果只是普通的打招呼或聊天，两个维度可以直接给 3 分以上。

        **决策指南：**
        - 如果 **正确性** 或 **流畅性** 评分 **都低于 3 分**，说明回复有重大缺陷，最终决策应输出“{Introspect}”（重试），并 **必须** 在 reason 字段中简要说明未通过的详细原因，以便下一轮生成时改进。
        - 如果 **正确性** 和 **流畅性** 评分 **都达到或超过 3 分**，说明回复质量足够高，最终决策应输出“{Finalize}”（接受），reason 字段可留空或返回 None。
    ...
    def _get_partial_variables(self) -> dict[str, Any]:
        return {
            ...
            'Finalize': IntrospectionClassification.Finalize.value,
        }
